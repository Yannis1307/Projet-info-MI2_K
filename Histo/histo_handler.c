...
// Data Aggregation Logic Identifies whether a CSV section refers to a Plant or an Intake.
...
//Recursive Tree Traversal and traverses the AVL tree (Right -> Root -> Left) to output data in descending order.
...
//Writes the CSV header appropriate for the selected mode and initiates and the recursive tree traversal to write the body of the data.
...
// Coordinates file opening (or stdin), CSV parsing, data aggregation via AVL tree,result writing, and memory cleanup.
